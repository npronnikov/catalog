...
- если передан `transitionMode`, `transitionName` или `transitionId` — требуется transition-сценарий;
- если активирован режим поиска (`jql` или структурированный), остальные модификаторы (`description`, `additionalFields`, `assignee`, `commentBody`, `transitionMode` и т.д.) игнорируются — выполняется только поиск.
...
4. Если результаты есть — сохранить массив `issues` с полными атрибутами каждой задачи
...
6. Не выполнять никаких других операций (создание, обновление, комментарий, transition) в режиме поиска
...
5. Сохранить результат по правилам режима 1 (шаги 3–6)
