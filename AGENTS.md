...
A **catalog of AI-powered development workflows, skills, and rules** — not application source code. It is consumed by AI coding agents (Qwen, Codex, Codex, Gigacode) via a platform that orchestrates multi-step development processes. The catalog is organized by teams and methodologies.
...
- **Coding agents**: Most entities target `qwen`; some have variants for `Codex`, `codex`, or `gigacode` (e.g., `restore-c4-architecture-java` has three agent-specific variants)
